    ...
    if (scene.meterStarting) {
      ...
      if (scene.distanceText) {
        scene.distanceText.setText(
          `Distance: ${parseInt(distance.toFixed(2) / 20)}\nAngle: ${parseInt(
            degrees
          )}`
        );
        scene.distanceText.setPosition(
          currLocation.x + 10,
          currLocation.y + 10
        );
      } else {
        scene.distanceText = scene.add.text(
          currLocation.x + 10,
          currLocation.y + 10,
          `Distance: ${distance.toFixed(2) / 20}`,
...
function findClosestPlayer(scene) {
  ...
  const closestGhost = findClosestPlayerFromGroup(
    scene.players.ghosts,
    scene.input
  );
  const closestMouse = findClosestPlayerFromGroup(
    scene.players.mice,
    scene.input
  );
