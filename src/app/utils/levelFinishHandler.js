...
function buildUIButtons(scene) {
    const buttonConfigs = [
        { x: 350, y: 500, key: 'retry_button', callback: () => resetScene(scene), scale: 0.3 },
        { x: 465, y: 500, key: 'next_level_button', callback: () => goToNextLevel(scene), scale: 0.3 },
        { x: 235, y: 500, key: 'menu_button', callback: () => showLevelSelector(scene), scale: 0.3 }
    ];

    buttonConfigs.forEach(config => {
        const button = scene.add.image(config.x, config.y, config.key)
            .setOrigin(0.5).setScale(config.scale).setDepth(105);
