...
export default class TheAbandonedRiverBaseLevel extends BaseLevel {
  ...
  createWorldProps(fenceLocations, keysLocations, treeFrame = true, tantArea = true) {
    // adding props to the map
    this.add.image(600 - 128, -270, 'tree').setOrigin(0).setScale(0.8).setDepth(5);
    this.add.image(700 - 128, -150, 'tree').setOrigin(0).setScale(0.8).setDepth(5);
    this.add.image(550 - 128, -330, 'tree').setOrigin(0).setScale(0.8).setDepth(4);
    // adding trees
    this.add.image(730- 128, 0, 'tree').setOrigin(0).setScale(0.8).setDepth(5)
    this.add.image(700- 128, 120, 'tree').setOrigin(0).setScale(0.8).setDepth(5)
    this.add.image(690- 128, 250, 'tree').setOrigin(0).setScale(0.9).setDepth(5)
    this.add.image(710- 128, 360, 'tree').setOrigin(0).setScale(0.8).setDepth(5)
    this.add.image(690- 128, 500, 'tree').setOrigin(0).setScale(0.7).setDepth(5)
    this.add.image(730- 128, 600, 'tree').setOrigin(0).setScale(0.7).setDepth(5)
    // left side trees
    ...
    this.add.sprite(60, 400, 'owl').setOrigin(0).setScale(0.13).play('owl_awake').setDepth(7);
    ...
    this.anims.create({
      key: 'fence',
      frameRate: 10,
      frames: this.anims.generateFrameNames('fence', { prefix: 'fence_0', start: 1, end: 6, suffix: '.png' }),
      repeat: 0
    });
    ...
    for (let i = 0; i < fenceLocations.length; i++)
    {
      ...
      for (let j = 0; j < 8; j++) {
        const fence = singleFenceGroup.create(fenceLocations[i].x + j * 20, fenceLocations[i].y, 'fence').setScale(0.5).setDepth(0);
    ...
    for(let i = 0; i < fences.length; i++)
    {
      ...
      {
        fenceCollider[i].push(this.physics.add.collider(this.players.ghosts[j], fences[i]));
      }
    }
    ...
    {
      let key = this.physics.add.image(keysLocations[i].x, keysLocations[i].y, 'key').setOrigin(0).setScale(0.4);
      ...
      {
        this.physics.add.overlap(this.players.ghosts[b].getAt(0), key, collectKey, null, this);
      }
      
      for (let b = 0; b < this.players.fish.length; b++) {
        this.physics.add.overlap(this.players.fish[b].getAt(0), key, collectKey, null, this);
      }
    }
  }
}
