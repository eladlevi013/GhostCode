...
const Home = () => {
  ...
  return (
    <div>
      <AnimatePresence>
        {isLoading && (
          <motion.div key="loading-screen" initial={{ opacity: 1 }}
            exit={{ opacity: 0 }} transition={{ duration: 0.5 }}
            className='loadingScreen'>
            <div className='loadingScreenText'>
              <img src='/assets/web/campfire_shadow.png' style={{ width: "205px",marginLeft: 
                '-109px',marginTop: "88px", position: "absolute", opacity: 0.25 }}/>
              <div style={{ position: "relative" }}>
                  <img src='/assets/web/campfire.gif' style={{ width: "175px", marginLeft:
      ...
      <div className='gamePageContainer'>
        ...
        <div>
          ...
          <button className={runningMode == 'run' ?
            'runButton' : 'resetButton'}onClick={handleClick}/>
          <button
            style={{backgroundImage: theme === 'dark'
              ?"url('/assets/UI/light_mode_button.png')"
              :"url('/assets/UI/dark_mode_button.png')"}}
              onClick={toggleTheme} className='themeToggleButton'/>
        </div>
      </div>
    </div>
  )
